...
LICENSE="BSD-2"
SLOT="0"
IUSE="debug elibc_glibc ncurses pam runit selinux static-libs unicode kernel_linux kernel_FreeBSD"
...
src_install() {
	[[ ${MAKE_ARGS} == "*LIBEXECDIR*" ]] || make_args
...
pkg_preinst() {
	...
	[[ -e ${EROOT}/etc/conf.d/net ]] && cp "${EROOT}"/etc/conf.d/net "${ED}"/etc/conf.d/
	...
	if [[ -L ${EROOT}/etc/runlevels/boot/clock ]] ; then
		...
		ln -snf "${EPREFIX}"/etc/init.d/${clock} "${EROOT}"/etc/runlevels/boot/${clock}
	fi
	if [[ -L ${EROOT}${LIBDIR}/rc/init.d/started/clock ]] ; then
		rm -f "${EROOT}${LIBDIR}"/rc/init.d/started/clock
		ln -snf "${EPREFIX}"/etc/init.d/${clock} "${EROOT}${LIBDIR}"/rc/init.d/started/${clock}
	fi
